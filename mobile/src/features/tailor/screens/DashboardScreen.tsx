...
const CARDS = [
  { icon: '✂', label: 'New', title: 'Available orders', desc: 'Orders matching your expertise', screen: 'AvailableOrders' as const, gradient: true },
  { icon: '📦', label: '', title: 'My orders', desc: 'Assigned and in-progress orders', screen: 'MyOrders' as const, gradient: false },
  { icon: '📊', label: '', title: 'Reports', desc: 'Earnings and completion stats', screen: 'Reports' as const, gradient: false },
  { icon: '👤', label: '', title: 'Profile', desc: 'Details, expertise and appearance', screen: 'Profile' as const, gradient: false },
];

export function DashboardScreen({ navigation }: TailorScreenProps<'Dashboard'>) {
              ...
              <Pressable key={card.title} onPress={() => navigation.navigate(card.screen)}>
                {card.gradient ? (
                  ...
                  >
                    ...
                    <Text style={{ fontSize: 10, color: 'rgba(255,255,255,0.75)', fontWeight: '600' as const, letterSpacing: 1.5, textTransform: 'uppercase' as const, marginBottom: 6 }}>
